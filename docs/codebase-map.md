...
The hook layers remain Java Build/SystemProperties, Bionic property APIs, uptime, selected native reads/AppLog, build-property mounts, and optional hide. Non-target and rejected companion responses close without FD exemption. A validated target retains the companion socket only when pre-specialization exemption succeeds; otherwise in-process presentation remains active while build-property mounts and optional hide are disabled for that process. Synthetic Wi-Fi/Bluetooth identities, sysfs-MAC reads, and SIM/carrier presentation are absent. General framework `device_name` settings still follow persona `MODEL`, while defensive property hiding continues to remove genuine telephony/Wi-Fi/Bluetooth identifiers where covered. The hooks do not guarantee late-loaded import coverage or cross-lens hardware/attestation virtualization. IPC IDs remain `GET_IDENTITY=2`, `DO_MOUNTS=3`, `DO_HIDE=4` with local integer framing in `config.hpp`.
