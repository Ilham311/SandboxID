...
The hook layers remain Java Build/SystemProperties, Bionic property APIs, uptime, selected native reads/AppLog, build-property mounts, and optional hide. Non-target and rejected companion responses close without FD exemption. A validated target retains the companion socket only when pre-specialization exemption succeeds; otherwise in-process presentation remains active while build-property mounts and optional hide are disabled for that process. Synthetic Wi-Fi/Bluetooth identities, sysfs-MAC reads, and SIM/carrier presentation are absent. General framework `device_name` settings still follow persona `MODEL`. Genuine OEM, telephony, Wi-Fi, and Bluetooth properties remain intact globally so non-target system processes retain required compatibility values; Java property hooks hide covered leaks only inside validated targets. Broad native PLT property/read hooks are default-off and forced off by the companion after an observed manager-specific commit failure. The hooks do not guarantee late-loaded import coverage or cross-lens hardware/attestation virtualization. IPC IDs remain `GET_IDENTITY=2`, `DO_MOUNTS=3`, `DO_HIDE=4` with local integer framing in `config.hpp`.
