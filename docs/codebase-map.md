...
Preflight obtains both lock domains and validates targets/tools/mode/property backend/package inventory before identity or device mutation. It classifies each unique base package as installed or absent. Prepare writes only pending state. Commit validates run, pending hash, and unchanged base state; stages overlays, backs up old canonical state, activates overlays, publishes canonical metadata, then consumes an override.
