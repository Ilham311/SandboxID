...
5. **apply** runs native property and framework application. Required presentation aliases remain fatal on backend failure. The explicitly classified OEM alias `ro.build.expect.baseband` is existing-only: genuine absence is a successful skip, but a failed attempted write remains fatal. Apply never deletes genuine global properties; target-only Java property hooks hide covered direct identifiers without removing compatibility values from non-target framework processes. Any fatal apply failure before irreversible work invokes restore, reapply, and exact verification.
