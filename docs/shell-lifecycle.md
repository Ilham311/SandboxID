...
```text
customize.sh
  preserve validated state/preferences -> permissions -> ABI CLI link
...
The installer describes the one-run effects and presentation boundary, then preserves an existing readable `target.txt` even when empty, `identity.mode`, optional carrier configuration, the remote-refresh preference, validated canonical/backup pairs, and the validated persona-cache pair. Copy failures abort rather than being reported as success. A narrowly scoped metadata-less legacy identity may be preserved for native migration; malformed or incomplete modern pairs are rejected.
...
## Optional acquisition: `autopif.sh`

The only public command is `refresh`. It is disabled unless `enable_remote_refresh` exists. It accepts an HTTPS URL on the explicit Google host allowlist, uses bounded connection/overall time and response size, refuses implicit redirects, and writes into a private temporary directory. It sends no canonical or local identifiers.

The adapter accepts exactly one explicit complete `SBX_PERSONA_V1` marker row and does not infer patch dates or fill required metadata heuristically. It writes provenance with schema/parser versions, positive retrieval time, exact runtime SDK, candidate SHA-256, adapter, and URL, then calls native `persona-import`. Network, TLS, HTTP, size, marker, schema, SDK, native validation, or publication failure leaves prior cache and override untouched. Remote data is an optional untrusted cache candidate, never the offline source of truth.
...
Canonical shell editing is disabled. `identity_persist` delegates supported local values to native `set-local` and six operational flags to `set-flag`; unsupported keys return usage failure. Property/framework helpers retain platform-specific fallback and retry behavior, but do not become canonical writers.
...
Action mode requires a valid inherited Action owner and `all --from-identity`. It strictly loads `GOOGLE_AID`, `WIFI_MAC`, `BLUETOOTH_ADDR`, `BLUETOOTH_NAME`, `BOOT_COUNT`, and `APPLOG_EPOCH` from canonical identity before mutating stores. Invalid/incomplete snapshots stop all rotation work. It never rewrites canonical local IDs in snapshot mode.

Standalone commands acquire their own mutation owner and publish desired supported local values through native `set-local` before external mutation. GAID accepts UUID-v4 or the all-zero opt-out sentinel and XML content is escaped. MACs must be locally administered unicast values; device names and decimal fields are bounded/validated.

Each component records `ok:<rc>`, `failed:<rc>`, or `unsupported:<rc>` in `debug/rotation.<run>` for Action. SSAID requests reboot only after verified deletion. GAID replacement restores the backed-up primary XML when later publication/ownership/context work fails. Wi-Fi, Bluetooth, framework, ownership, context, and service/interface restoration errors contribute to status instead of being silently converted to success. Cleanup restores SELinux and releases only owned standalone locks.

Carrier apply/disable delegates to the native carrier transaction. The strict config and carrier-owned identity fields are co-published or restored together; `carriers.tsv` is optional WebUI convenience data, not state authority.
...
`build.sh` requires core runtime files, the pinned verified Zygisk header, four ABI outputs, and a complete verified resetprop set when bundled. It runs the package-manifest smoke test and performs no persona acquisition. `module.prop` is a hard blocker. Optional `personas.tsv` and `carriers.tsv` are copied only when present.
