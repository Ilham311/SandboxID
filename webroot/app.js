...
function onTab(id) {
  ...
  else if (id === 'wp') loadWp();
  else if (id === 'log') loadLog();
}

/* ---------- Work Profile ---------- */
async function loadWp() {
  document.getElementById('wpLog').textContent = 'Loading status...';
  const r = await safeExec(`sh ${shq(MODDIR)}/check_profile.sh 2>&1`);
  document.getElementById('wpLog').textContent = r.ok ? r.out : (r.err && r.err.message || 'error');
}

async function wpAction(script, arg = '') {
  const logEl = document.getElementById('wpLog');
  logEl.textContent = `Running ${script}...\n`;
  const cmd = `sh ${shq(MODDIR)}/${script} ${shq(arg)} 2>&1`;
  const r = await safeExec(cmd);
  logEl.textContent += (r.ok ? r.out : (r.err && r.err.message || 'error'));
  if (script !== 'check_profile.sh') {
    setTimeout(loadWp, 1500); // refresh status after action
  }
}

document.getElementById('wpCreate').addEventListener('click', () => wpAction('create_profile.sh'));
document.getElementById('wpRemove').addEventListener('click', () => wpAction('remove_profile.sh'));
document.getElementById('wpCheck').addEventListener('click', () => wpAction('check_profile.sh'));
document.getElementById('wpAddTarget').addEventListener('click', () => {
  const pkg = document.getElementById('wpTargetPkg').value.trim();
  if (!pkg) {
    toast('Please enter a package name', 'error');
    return;
  }
