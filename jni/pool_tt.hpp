// ternak_tt v2 — pool_tt.hpp
// Item #5: diversified device pool (Pixel + Samsung + Xiaomi + OnePlus)
//          + jitter helper for security_patch so cluster isn't fingerprintable.
//
// Vs v1: was 14 Pixel-only entries with 2 clustered security_patch dates.
// Now:   22 entries across 4 brands with 8 distinct patch dates.
...
struct PixelEntry {           // name kept for source compatibility
    const char* model;
    const char* device;
    const char* product;
    const char* board;
    ...
    const char* brand;         // NEW: "google", "samsung", "xiaomi", "OnePlus"
    const char* manufacturer;  // NEW: "Google", "Samsung", "Xiaomi", "OnePlus"
};
...
static constexpr PixelEntry TT_POOL[] = {
    ...
    // ---- Samsung Galaxy A5x / A7x (mid-range, high volume) ----
    ...
    // ---- Google Pixel 10 (Tensor G5) — latest generation ----
...
static constexpr size_t TT_POOL_SIZE = sizeof(TT_POOL) / sizeof(TT_POOL[0]);
