...
- **Debug variant** auto-captures per-boot session logs (filename pattern `session-YYYYMMDD-HHMMSS.log`), produces a small chat-shareable summary on Action tap, and keeps a persistent `crashes.log` journal.
...
- Automatic per-session logging to `/data/adb/modules/ternak_tt/debug/` (filename pattern: `session-YYYYMMDD-HHMMSS.log`)
