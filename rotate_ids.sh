...
set_gaid_value() {
    ...
    command -v am >/dev/null 2>&1 && am kill --user 0 com.google.android.gms >/dev/null 2>&1
    sleep 1
...
cmd_status() {
    ...
    if command -v settings >/dev/null 2>&1; then
        log_info "Settings.Global.advertising_id  = $(mask_id "$(settings get --user 0 global advertising_id 2>/dev/null)")"
        log_info "Settings.Global.device_name     = $(settings get --user 0 global device_name 2>/dev/null)"
        log_info "Settings.Global.bluetooth_name  = $(settings get --user 0 global bluetooth_name 2>/dev/null)"
    fi
